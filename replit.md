...
- **Mobile — Tema Dark SaaS**: fundo escuro `#0F0F14`, cards `#1A1A24`, inputs `#1E1E28`, texto primário `#F0F0F5`, secundário `#D1D1DB`, bordas `#2A2A35`/`#252530`. Headers gradient roxo escuro `LinearGradient ["#6D28D9","#4C1D95","#3B0764"]`. BottomNav custom (`components/BottomNav.tsx`): barra dark `#1A1A24` com `borderTopRadius: 22`, tab ativa com círculo roxo `#6D28D9` (56px, marginTop -20), inativas cinza `#6B7280`. **Dashboard SaaS** (`index.tsx`): greeting header, plan badge, grid 2x2 de métricas (Total Bots, Online, Offline, Mensagens), gráfico de barras semanal, grid 2x2 ações rápidas, lista de bots com status pills, atividade recente. **Builder** (`builder/[id].tsx`): interface de lista (sem SVG/canvas) — cards com borda colorida por tipo, conexões inline, botões SIM/NÃO para condições, templates prontos, toque para editar/conectar. Status bgs: verde `#0D2818`, amarelo `#2D2506`, vermelho `#2D0A0A`, roxo `#150F2A`. `constants/colors.ts` define paleta dark. QR code box mantém fundo branco.
