...
- **Mobile — Tema Dark Mode**: fundo escuro `#0F0F14`, cards `#1A1A24`, inputs `#1E1E28`, texto primário `#F0F0F5`, secundário `#D1D1DB`, bordas `#2A2A35`/`#252530`. Headers gradient roxo `LinearGradient ["#7C3AED","#6D28D9"]`. BottomNav custom (`components/BottomNav.tsx`): barra dark `#1A1A24` com `borderTopRadius: 22`, tab ativa com círculo verde `#2FAE8F` (56px, marginTop -20), inativas cinza `#D1D1DB`. Status bgs: verde `#0D2818`, amarelo `#2D2506`, vermelho `#2D0A0A`, roxo `#1E1635`. `constants/colors.ts` define paleta dark. Todas as 12 telas dark: `_layout`, `index`, `bots`, `payments`, `plans`, `settings`, `admin`, `login`, `register`, `bot/[id]`, `bot/settings/[id]`, `+not-found`. Builder visual mantém tema escuro (contraste para canvas). QR code box mantém fundo branco para leitura.
