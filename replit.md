...
- **Mobile — Tema Dark SaaS**: fundo escuro `#0F0F14`, cards `#1A1A24`, inputs `#1E1E28`, texto primário `#F0F0F5`, secundário `#D1D1DB`, bordas `#2A2A35`/`#252530`. Headers gradient roxo `LinearGradient ["#7C3AED","#6D28D9"]`. BottomNav custom (`components/BottomNav.tsx`): barra dark `#1A1A24` com `borderTopRadius: 22`, tab ativa com círculo roxo `#7C3AED` (56px, marginTop -20), inativas cinza `#6B7280`. **Dashboard SaaS** (`index.tsx`): greeting header, plan badge, grid 2x2 de métricas (Total Bots, Online, Offline, Mensagens), gráfico de barras semanal, grid 2x2 ações rápidas (Criar Bot/Builder/Recarregar/Planos), lista de bots com status pills, atividade recente. Status bgs: verde `#0D2818`, amarelo `#2D2506`, vermelho `#2D0A0A`, roxo `#1E1635`. `constants/colors.ts` define paleta dark. Todas as 12 telas dark. Builder visual mantém tema escuro. QR code box mantém fundo branco.
- **Cores consistentes**: primary `#7C3AED`, darker `#6D28D9`, purple bg `#1E1635`. Status: online `#22C55E`, warning `#F59E0B`, error `#EF4444`, offline `#9CA3AF`
