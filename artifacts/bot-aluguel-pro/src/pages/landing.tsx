...
function FAQItem({ q, a, id }: { q: string; a: string; id: string }) {
  ...
  return (
    <motion.div
      variants={fadeInUp}
      className="border border-white/5 rounded-xl overflow-hidden bg-card hover:border-white/10 transition-colors"
    >
      ...
      <div id={`faq-${id}`} role="region" className={`overflow-hidden transition-all duration-300 ${open ? "max-h-40 pb-5" : "max-h-0"}`}>
        <p className="px-5 text-muted-foreground text-sm leading-relaxed">{a}</p>
      </div>
    </motion.div>
  );
}
...
export default function LandingPage() {
  ...
  return (
    <div className="min-h-screen bg-background text-foreground overflow-x-hidden">
      <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled ? "bg-background/95 backdrop-blur-lg border-b border-white/10 shadow-lg" : "bg-transparent"}`}>
        <div className="max-w-7xl mx-auto px-4 h-16 flex items-center justify-between">
          ...
          <nav className="hidden md:flex items-center gap-6">
            ...
            <a href="#como-funciona" className="text-sm text-muted-foreground hover:text-white transition-colors">Como Funciona</a>
            <a href="#planos" className="text-sm text-muted-foreground hover:text-white transition-colors">Planos</a>
      ...
      <section className="relative pt-32 pb-24 px-4 overflow-hidden">
        ...
        <div className="relative max-w-5xl mx-auto text-center">
          ...
          >
            <Button asChild size="lg" className="bg-primary hover:bg-primary/90 text-white px-8 py-6 text-base font-semibold group shadow-xl shadow-primary/20">
              <Link href="/register">
                Criar conta gratis
          ...
          <motion.p className="mt-8 text-sm text-muted-foreground" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.5 }}>
        ...
        >
          {[
            ...
            <div key={stat.label} className="text-center p-4 rounded-xl border border-white/5 bg-card/50 backdrop-blur-sm">
      ...
      <section id="recursos" className="py-24 px-4 bg-card/30">
        <div className="max-w-6xl mx-auto">
          <motion.div className="text-center mb-16" initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
            <Badge className="mb-4 border-primary/30 bg-primary/10 text-primary text-xs px-3 py-1">Recursos</Badge>
            <h2 className="text-3xl md:text-4xl font-bold text-white mb-4">Tudo que voce precisa para automatizar</h2>
            <p className="text-muted-foreground max-w-xl mx-auto">Uma plataforma completa para criar e gerenciar seus bots de WhatsApp sem precisar de um tecnico.</p>
          </motion.div>
          <motion.div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6" variants={staggerContainer} initial="hidden" whileInView="show" viewport={{ once: true }}>
            {features.map((feature) => (
              <motion.div key={feature.title} variants={fadeInUp}
                className="p-6 rounded-xl border border-white/5 bg-card hover:border-primary/20 hover:bg-primary/[0.02] transition-all duration-300 group"
              >
      ...
      <section id="como-funciona" className="py-24 px-4">
        <div className="max-w-5xl mx-auto">
          <motion.div className="text-center mb-16" initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
            <Badge className="mb-4 border-accent/30 bg-accent/10 text-accent text-xs px-3 py-1">Como Funciona</Badge>
            <h2 className="text-3xl md:text-4xl font-bold text-white mb-4">3 passos simples</h2>
            <p className="text-muted-foreground max-w-xl mx-auto">Do cadastro ao bot funcionando em menos de 10 minutos.</p>
          </motion.div>
          <motion.div className="grid md:grid-cols-3 gap-8" variants={staggerContainer} initial="hidden" whileInView="show" viewport={{ once: true }}>
            {steps.map((step, i) => (
              <motion.div key={step.num} variants={fadeInUp} className="relative text-center group">
                ...
                <div className="h-24 w-24 rounded-2xl bg-primary/10 border border-primary/20 flex items-center justify-center mx-auto mb-6 group-hover:bg-primary/20 group-hover:scale-110 transition-all duration-300">
      ...
      <section id="planos" className="py-24 px-4 bg-card/30">
        <div className="max-w-5xl mx-auto">
          <motion.div className="text-center mb-16" initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
            <Badge className="mb-4 border-primary/30 bg-primary/10 text-primary text-xs px-3 py-1">Planos</Badge>
            ...
            {plans.map((plan) => (
              <motion.div key={plan.id} variants={fadeInUp}
                className={`relative p-6 rounded-xl border transition-all duration-300 hover:scale-[1.02] ${
                  plan.popular ? "border-primary bg-primary/5 shadow-xl shadow-primary/10" : "border-white/5 bg-card hover:border-white/10"
                }`}
              >
                {plan.popular && (
                  <Badge className="absolute -top-3 left-1/2 -translate-x-1/2 bg-primary text-white border-0 shadow-lg">Mais Popular</Badge>
                )}
                <div className="mb-6">
                ...
                <Button asChild className={`w-full ${plan.popular ? "bg-primary hover:bg-primary/90 text-white shadow-lg shadow-primary/20" : "border-white/10 bg-white/5 hover:bg-white/10 text-white"}`} variant={plan.popular ? "default" : "outline"}>
      ...
      <section className="py-24 px-4">
        <div className="max-w-6xl mx-auto">
          <motion.div className="text-center mb-16" initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
            <Badge className="mb-4 border-yellow-500/30 bg-yellow-500/10 text-yellow-400 text-xs px-3 py-1">Depoimentos</Badge>
            <h2 className="text-3xl md:text-4xl font-bold text-white mb-4">O que nossos usuarios dizem</h2>
            <p className="text-muted-foreground max-w-xl mx-auto">Veja como o BotAluguel Pro esta transformando negocios.</p>
          </motion.div>
          <motion.div className="grid md:grid-cols-2 gap-6" variants={staggerContainer} initial="hidden" whileInView="show" viewport={{ once: true }}>
            {testimonials.map((t) => (
              <motion.div key={t.name} variants={fadeInUp}
                className="p-6 rounded-xl border border-white/5 bg-card hover:border-white/10 transition-all duration-300"
              >
                <div className="flex gap-1 mb-4">
                  {Array.from({ length: 5 }).map((_, i) => (
                    <Star key={i} className={`h-4 w-4 ${i < t.stars ? "text-yellow-400 fill-yellow-400" : "text-white/10"}`} />
                  ))}
                </div>
      ...
      <section id="faq" className="py-24 px-4 bg-card/30">
        <div className="max-w-3xl mx-auto">
          <motion.div className="text-center mb-16" initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
            <Badge className="mb-4 border-primary/30 bg-primary/10 text-primary text-xs px-3 py-1">FAQ</Badge>
            <h2 className="text-3xl md:text-4xl font-bold text-white mb-4">Perguntas Frequentes</h2>
            <p className="text-muted-foreground">Tudo que voce precisa saber sobre o BotAluguel Pro.</p>
          </motion.div>
      ...
      <section className="py-24 px-4">
        <div className="max-w-4xl mx-auto">
          <motion.div
            className="relative rounded-2xl border border-primary/20 bg-gradient-to-br from-primary/10 via-card to-accent/10 p-10 sm:p-16 text-center overflow-hidden"
          ...
          >
            ...
            <div className="relative">
              ...
              <p className="text-muted-foreground mb-8 max-w-lg mx-auto">Junte-se a centenas de usuarios que ja usam o BotAluguel Pro para automatizar atendimentos e grupos.</p>
              <div className="flex flex-col sm:flex-row gap-4 justify-center">
                <Button asChild size="lg" className="bg-primary hover:bg-primary/90 text-white px-10 py-6 text-base font-semibold shadow-xl shadow-primary/20 group">
                  <Link href="/register">
        ...
        <div className="max-w-6xl mx-auto px-4 py-16">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8 mb-12">
            <div className="col-span-2 md:col-span-1">
              ...
              <p className="text-muted-foreground text-sm leading-relaxed">Plataforma SaaS de bots WhatsApp para automatizar seus atendimentos e grupos.</p>
            </div>
            <div>
              <h4 className="text-white font-semibold mb-4 text-sm">Produto</h4>
              <ul className="space-y-2">
                <li><a href="#recursos" className="text-muted-foreground text-sm hover:text-white transition-colors">Recursos</a></li>
                <li><a href="#planos" className="text-muted-foreground text-sm hover:text-white transition-colors">Planos</a></li>
                <li><a href="#como-funciona" className="text-muted-foreground text-sm hover:text-white transition-colors">Como Funciona</a></li>
                <li><a href="#faq" className="text-muted-foreground text-sm hover:text-white transition-colors">FAQ</a></li>
              </ul>
            </div>
            <div>
              <h4 className="text-white font-semibold mb-4 text-sm">Conta</h4>
              <ul className="space-y-2">
                <li><Link href="/register" className="text-muted-foreground text-sm hover:text-white transition-colors">Criar Conta</Link></li>
                <li><Link href="/login" className="text-muted-foreground text-sm hover:text-white transition-colors">Entrar</Link></li>
                <li><Link href="/dashboard" className="text-muted-foreground text-sm hover:text-white transition-colors">Dashboard</Link></li>
              </ul>
            </div>
            <div>
              <h4 className="text-white font-semibold mb-4 text-sm">Contato</h4>
              <ul className="space-y-2">
                <li className="flex items-center gap-2 text-muted-foreground text-sm"><Mail className="h-3.5 w-3.5" /> suporte@botaluguel.pro</li>
                <li className="flex items-center gap-2 text-muted-foreground text-sm"><MessageSquare className="h-3.5 w-3.5" /> WhatsApp</li>
                <li className="flex items-center gap-2 text-muted-foreground text-sm"><Globe className="h-3.5 w-3.5" /> botaluguel.pro</li>
              </ul>
            </div>
          </div>
          <div className="border-t border-white/5 pt-8 flex flex-col md:flex-row items-center justify-between gap-4">
            <p className="text-muted-foreground text-xs">2024-2026 BotAluguel Pro. Todos os direitos reservados.</p>
            <div className="flex items-center gap-4">
              <span className="text-muted-foreground/50 text-xs">Termos de Uso</span>
              <span className="text-muted-foreground/50 text-xs">Privacidade</span>
            </div>
          </div>
        </div>
      </footer>
