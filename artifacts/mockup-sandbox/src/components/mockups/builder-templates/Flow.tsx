...
import { Zap, MessageCircle, GitBranch, MessageSquare, Save, ChevronLeft, Plus, ZoomIn, ZoomOut, Home, Sparkles, BarChart3, Settings, User } from "lucide-react";
    ...
    <div className="w-[390px] h-[844px] flex flex-col overflow-hidden" style={{ backgroundColor: "#0C0C11", fontFamily: "Inter, system-ui, sans-serif" }}>
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b" style={{ borderColor: "#20202B", backgroundColor: "#13131D" }}>
        ...
        <div className="flex items-center gap-2">
          <div className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-xl border" style={{ borderColor: "#F59E0B40", backgroundColor: "#F59E0B10" }}>
            <div className="w-1.5 h-1.5 rounded-full" style={{ backgroundColor: "#F59E0B" }} />
            <span className="text-[10px] font-semibold" style={{ color: "#F59E0B" }}>não salvo</span>
          </div>
          ...
          <div className="w-8 h-8 rounded-2xl flex items-center justify-center border" style={{ backgroundColor: "#20202B", borderColor: "#20202B" }}>
            <User size={14} color="#555575" />
          </div>
        </div>
      </div>
      ...
      {/* ── FOOTER: zoom row + bottom nav ── */}
      <div className="border-t" style={{ borderColor: "#20202B", backgroundColor: "#13131D" }}>
        {/* Zoom */}
        <div className="flex items-center justify-between px-4 py-2 border-b" style={{ borderColor: "#20202B" }}>
          <span className="text-[10px]" style={{ color: "#444460" }}>4 blocos · 3 conexões</span>
        ...
        <div className="flex items-center justify-around px-2 pt-2 pb-4">
          {NAV_TABS.map(t => {
            ...
            return (
              <button key={t.id} onClick={() => setActiveTab(t.id)} className="flex flex-col items-center gap-1 px-4 py-1.5 rounded-2xl" style={isActive ? { backgroundColor: "#6D28D920" } : {}}>
